...
pipeline {
    ...
    stages {
        ...
        stage('Deploy QA') {
            steps {
                echo " deploying containter image to qa started..."

                script {
                    //Replace variables for dev environment
                    replaceValuesInFile('config_environment/config_qa.properties', 'src/main/resources/application-template.properties', 'src/main/resources/application.properties')
                    ...
                    withEnv(["PATH=${ocDir}:$PATH"]) {
                        ...
                        sh "oc delete cm ${OPENSHIFT_APP_NAME}-config --ignore-not-found=true -n ${OPENSHIFT_NAMESPACE_QA}"

                        sh "oc create cm ${OPENSHIFT_APP_NAME}-config --from-file=./src/main/resources/application.properties -n ${OPENSHIFT_NAMESPACE_QA}"

                        sh "oc set image dc/${OPENSHIFT_APP_NAME} ${OPENSHIFT_APP_NAME}=${OPENSHIFT_NAMESPACE_DEV}/${OPENSHIFT_APP_NAME}:${tagImage}" +
                                " --source=imagestreamtag -n ${OPENSHIFT_NAMESPACE_QA}"
                        sh "oc rollout latest dc/${OPENSHIFT_APP_NAME} -n ${OPENSHIFT_NAMESPACE_QA}"

                        def dc_version = sh(script: "oc get dc/${OPENSHIFT_APP_NAME} -o=yaml -n ${OPENSHIFT_NAMESPACE_QA} | grep 'latestVersion'| cut -d':' -f 2",
                                returnStdout: true).trim();
                        echo "Current Version DeploymentConfig ${dc_version}"
